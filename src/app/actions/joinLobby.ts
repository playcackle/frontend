...
export const joinGameroom = async (playerId: string) => {
  ...
  const clientBackendUrl = process.env.NEXT_PUBLIC_BACKEND_JOIN_URL || 'http://localhost:8000';
  
  if (gameroom.game_ws_url) {
    // Replace the base URL part with client-accessible URL
    gameroom.game_ws_url = gameroom.game_ws_url.replace(/^https?:\/\/[^\/]+/, clientBackendUrl);
  }
  if (gameroom.chat_ws_url) {
    gameroom.chat_ws_url = gameroom.chat_ws_url.replace(/^https?:\/\/[^\/]+/, clientBackendUrl);
  }
